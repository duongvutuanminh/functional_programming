...
def play_marker(board, player, position):
    while check_existing_position(board,position):
        print("You must choose another position!")
        print(f"Player with marker {player} has to go again")
        position_new = input()
        play_marker(board,player,position_new)
...
def check_existing_position(board,i):
    return board[int(i)] == "X" or board[int(i)] == "O"
...
def main():
    ...
    while game_on:
        if player == "X":
            ...
            if win_check(current_board,player):
                print(f"Player with marker O has won the game")
                game_on = False
            else:
                if check_full_board(current_board):
                    print("Draw!")
